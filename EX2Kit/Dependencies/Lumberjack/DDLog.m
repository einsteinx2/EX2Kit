...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		numProcessors = MAX([NSProcessInfo processInfo].processorCount, 1);
             ...
             options:(DDLogMessageOptions)optionsMask
{
	if ((self = [super init]))
	{
		...
		queueLabel = dd_str_copy(dispatch_queue_get_label(DDLogCurrentQueue));
...
- (id <DDLogFormatter>)logFormatter
{
	...
	dispatch_queue_t currentQueue = DDLogCurrentQueue;
